...
No Ruby installation required — this uses the official Jekyll Docker image. Run this from the **repo root** (not from `site/`) — `_config.yml`, `_posts/`, `_layouts/`, etc. all live at the root; only the `Gemfile` sits in `site/`, so it's pointed at explicitly via `BUNDLE_GEMFILE`.

```
docker run --rm -it -p 4000:4000 -v "$PWD":/srv/jekyll -e BUNDLE_GEMFILE=/srv/jekyll/site/Gemfile jekyll/jekyll:latest sh -c "bundle install && bundle exec jekyll serve --host 0.0.0.0 --livereload --force_polling"
```
